...
using GroupInvest.Microservices.Participantes.API.Helpers.Swagger;
    ...
    public static class SwaggerSetting
    {
        public static IServiceCollection ConfigureSwagger(this IServiceCollection services)
        {
            services.AddSwaggerGen(config =>
            {
                ...
                config.SwaggerDoc("v1.0", new Microsoft.OpenApi.Models.OpenApiInfo { Title = "GroupInvest Participantes - versão 1.0", Version = "v1.0" });
                //config.SwaggerDoc("v2.0", new Microsoft.OpenApi.Models.OpenApiInfo { Title = "GroupInvest Participantes - versão 2.0", Version = "v2.0" });
                ...
                config.AddSecurityDefinition("oauth2", new OpenApiSecurityScheme
                {
                    ...
                    Flows = new OpenApiOAuthFlows
                    {
                        AuthorizationCode = new OpenApiOAuthFlow
                        {
                            ...
                            {
                                { UserSecretsHelper.APIName, "API de Participantes" }
                            }
                        }
                    }
                });
        ...
        public static IApplicationBuilder ConfigureSwaggerUI(this IApplicationBuilder app)
        {
            ...
            {
                config.SwaggerEndpoint("/swagger/v1.0/swagger.json", "Participantes API v1.0");

                // configura outras versões da api para ser selecionada no Swagger
                //config.SwaggerEndpoint("/swagger/v2.0/swagger.json", "Participantes API v2.0");
