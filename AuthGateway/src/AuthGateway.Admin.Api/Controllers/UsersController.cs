...
using AuthGateway.Admin.Api.Configuration.Constants;
using AuthGateway.Admin.Api.Dtos.Roles;
using AuthGateway.Admin.Api.Dtos.Users;
using AuthGateway.Admin.Api.ExceptionHandling;
using AuthGateway.Admin.Api.Helpers.Localization;
using AuthGateway.Admin.Api.Resources;
using Skoruba.IdentityServer4.Admin.BusinessLogic.Identity.Dtos.Identity;
using Skoruba.IdentityServer4.Admin.BusinessLogic.Identity.Services.Interfaces;

namespace AuthGateway.Admin.Api.Controllers
{
        ...
        where TRoleClaimDto : RoleClaimDto<TKey>
    {
        ...
        /// <summary>
        /// Cria um usuário no AuthGateway com a Role AppUser
        /// AppUser Role Id = 061a607c-9614-45d3-b6b0-33331be6d6e9
        /// </summary>
        /// <param name="appUser">App User</param>
        /// <returns></returns>
        [HttpPost("RegisterAppUser")]
        public async Task<ActionResult<TUserDto>> RegisterAppUser([FromBody] AppUserDto appUser)
        {
            if (!EqualityComparer<TKey>.Default.Equals(appUser.User.Id, default))
                return BadRequest(_errorResources.CannotSetId());

            var (_, userId) = await _identityService.CreateUserAsync(appUser.User);
            var createdUser = await _identityService.GetUserAsync(userId.ToString());

            // adiciona a role de usuário do aplicativo ao usuário cadastrado para garantir o acesso ao aplicativo
            var role = await _identityService.GetRoleAsync("061a607c-9614-45d3-b6b0-33331be6d6e9");
            var userRole = new UserRoleApiDto<TKey>
            {
                UserId = userId,
                RoleId = role.Id
            };

            var userRolesDto = _mapper.Map<TUserRolesDto>(userRole);
            await _identityService.CreateUserRoleAsync(userRolesDto);

            // associa a senha ao usuário
            appUser.Password.UserId = createdUser.Id;
            var userChangePasswordDto = _mapper.Map<TUserChangePasswordDto>(appUser.Password);
            await _identityService.UserChangePasswordAsync(userChangePasswordDto);
